...
class DBHelper {
  ...
  Future<Deyimler> save(Deyimler employee) async {
  ...
  Future<List<Deyimler>> getEmployees() async {
    var dbClient = await db;
    List<Map> maps = await dbClient.query(TABLE, columns: [ID, NAME]);
    //List<Map> maps = await dbClient.rawQuery("SELECT * FROM $TABLE");
    List<Deyimler> employees = [];
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        employees.add(Deyimler.fromMap(maps[i]));
      }
    }
    return employees;
  }
  ...
  Future<int> update(Deyimler employee) async {
    var dbClient = await db;
